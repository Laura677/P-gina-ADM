...
function buscar() {
  var dados = JSON.parse(localStorage.getItem("tds")) || [];
  let login = document.querySelector("#pesquisar").value.trim();
  let encontrou = false;

  for (let i = 0; i < dados.length; i++) {
    // Garante que o dado existe e tem a propriedade "nome"
    if (dados[i] && dados[i].nome === login) {
      document.querySelector("#id").value = dados[i].id;
      document.querySelector("#login").value = dados[i].nome;
      document.querySelector("#senha").value = dados[i].senha;
      encontrou = true;
...
function tabela1() {
  var dados = JSON.parse(localStorage.getItem("tds")) || [];
  var tbody = document.querySelector("#tabela-corpo");
